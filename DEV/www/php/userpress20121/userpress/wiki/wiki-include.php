...
function up546E_Wiki() {
	...
	if ( ! isset ( $this->_options['default']['modal_links'] ) ) {
		$this->_options['default']['modal_links']  =  0;
	}
...
function install() {
	...
	*/
	...
	if ( ! isset ( $this->_options['default']['modal_links'] ) ) {
		$this ->_options['default']['modal_links']  =  FALSE;
	}
...
function admin_menu_user_guide() {
	...
	if (is_array($submenu)) {
		if ($submenu['edit.php?post_type=userpress_wiki']) {
			$array_keys = array_keys($submenu['edit.php?post_type=userpress_wiki']);
			$key = end($array_keys) + 1; 
...
function up546E_add_subscribe_button_to_title($title, $id=null) {
	global $post;
